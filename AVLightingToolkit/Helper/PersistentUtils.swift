...
class PersistentUtils {
    ...
    func seedLightPattern() {
        ...
        let lightpatterns = [
            ...
            (imageFilename: "20181125_10(2)", name: "Waiting as movement + identification with colours", position:12, code: "var mapToNative = function(leds, num) { return test.map(function(led, index) { if (loop <= 0.8) { var pos1 = parseInt((loop - 0)*(8-1)/(0.8 - 0) + 1, 10); var pos2 = parseInt((loop - 0)*(12-19)/(0.8 - 0) + 19, 10); if (index == pos1) { return LED.setAllWithRedGreenBlue(color1.getRed(), color1.getGreen(), color1.getBlue()); } else if (index == (pos1 - 1) || index == (pos1 + 1)) { return LED.setAllWithRedGreenBlue(color1.getRed()*0.5, color1.getGreen()*0.5, color1.getBlue()*0.5); } else if (index == pos2) { return LED.setAllWithRedGreenBlue(color1.getRed(), color1.getGreen(), color1.getBlue()); }else if (index == (pos2 - 1) || index == (pos2 + 1)) { return LED.setAllWithRedGreenBlue(color1.getRed()*0.5, color1.getGreen()*0.5, color1.getBlue()*0.5); } else return LED.setAllWithRedGreenBlue(0,0,0); } else if (index == 10) { return LED.setAllWithRedGreenBlue(color1.getRed()*loop, color1.getGreen()*loop, color1.getBlue()*loop); } else return LED.setAllWithRedGreenBlue(0,0,0); }) };", color1: UIColor.red, color2: nil),
