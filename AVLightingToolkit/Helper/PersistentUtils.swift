...
class PersistentUtils {
    ...
    func seedLightPattern() {
        ...
        let lightpatterns = [
            ...
            (imageFilename: "20190425", name: "Speed as colours", position:3, code: "var mapToNative = function(leds, num) { return test.map(function(led, index) { return LED.setAllWithRedGreenBlue(interpolate(color1.getRed(), color2.getRed(), loop, 1), interpolate(color1.getGreen(), color2.getGreen(), loop, 1), interpolate(color1.getBlue(), color2.getBlue(), loop, 1)); }) }; function interpolate(start, end, step, last) { return (end - start) * step / last + start; }", color1: UIColor.green, color2: UIColor.red),
