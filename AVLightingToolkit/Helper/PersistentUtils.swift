...
class PersistentUtils {
    ...
    func seedLightPattern() {
        ...
        let lightpatterns = [
            ...
            (imageFilename: "20181125_4(2)", name: "Speed as spatial information + effects", position:4, code: "var mapToNative = function(leds, num) { return test.map(function(led, index) { if (index >= 7 + parseInt(loop * (10 - 6)) && index <= 10) { return LED.setAllWithRedGreenBlue(interpolate(0.25*color1.getRed(), color1.getRed(), index - 6, 4), interpolate(0.25*color1.getGreen(), color1.getGreen(), index - 6, 4), interpolate(0.25*color1.getBlue(), color1.getBlue(), index - 6, 4)); } else if (index > 10 && index <= 13 - parseInt(loop * (14 - 10))) { return LED.setAllWithRedGreenBlue(interpolate(color1.getRed(), 0.25*color1.getRed(), index - 10, 4), interpolate(color1.getGreen(), 0.25*color1.getGreen(), index - 10, 4), interpolate(color1.getBlue(), 0.25*color1.getBlue(), index - 10, 4)); } else return LED.setAllWithRedGreenBlue(0, 0, 0); }) }; function interpolate(start, end, step, last) { return (end - start) * step / last + start; }", color1: UIColor.purple, color2: nil),
